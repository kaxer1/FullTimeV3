...
export class MetricaVacacionesComponent implements OnInit {
  ...
  llamarGraficaOriginal() {
    ...
    // console.log('LOCAL vacaciones: ');
    if (local === null) {
      this.restGraficas.EmpleadoVacaciones().subscribe(res => {
        // console.log('************* vacaciones Micro **************');
  ...
  ValidarRangofechas(form) {
    ...
    if (f_i < f_f) {
      ...
      if (f_i.getFullYear() === f_f.getFullYear()) {
        ...
        this.restGraficas.EmpleadoVacacionesMacro(this.f_inicio_req, this.f_final_req).subscribe(res => {
          console.log('#################### VACACIONES Macro Retorna #######################');
          console.log(res);
          this.vacaciones = res
        });
      } else {
  ...
  getDocumentDefinicion() {
    return {
      ...
      content: [
        ...
        { text: 'Métrica Vacaciones', bold: true, fontSize: 20, alignment: 'center', margin: [0, -30, 0, 10] },
