...
export class MetricaAtrasosComponent implements OnInit {
  ...
  llamarGraficaOriginal() {
    ...
    this.chartDom = document.getElementById('charts_atraso_macro') as HTMLCanvasElement;
  ...
  ValidarRangofechas(form) {
    ...
    if (f_i < f_f) {
      ...
      if (f_i.getFullYear() === f_f.getFullYear()) {
        ...
        this.restGraficas.EmpleadoAtrasosMacro(this.f_inicio_req, this.f_final_req).subscribe(res => {
          console.log('#################### Atrasos Macro Retorna #######################');
          console.log(res);
          this.atrasos = res;
          this.thisChart.setOption(res);
        });
  ...
  ImprimirDatos() {
    let meses = this.atrasos.xAxis.data
    let valor = this.atrasos.series.data
    let datos: any = [];
