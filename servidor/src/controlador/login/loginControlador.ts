...
class LoginControlador {
  ...
  public async ValidarCredenciales(req: Request, res: Response) {
    try {
      ...
      const SUC_DEP = await pool.query('SELECT c.id_departamento, c.id_sucursal, s.id_empresa FROM empl_contratos AS e, empl_cargos AS c, sucursales AS s WHERE e.id_empleado = $1 AND c.id_empl_contrato = e.id AND c.id_sucursal = s.id ORDER BY c.fec_inicio DESC LIMIT 1', [USUARIO.rows[0].id_empleado]);
