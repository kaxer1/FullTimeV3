...
class EmpleadoControlador {
  ...
  public async getTitulosDelEmpleado(req: Request, res: Response): Promise<any> {
    ...
    const unEmpleadoTitulo = await pool.query('SELECT et.observacion As observaciones, ct.nombre, nt.nombre as nivel FROM empleados as e, empl_titulos AS et, cg_titulos AS ct, nivel_titulo AS nt WHERE e.id = $1 and et.id_titulo = ct.id and ct.id_nivel = nt.id', [id_empleado]);
