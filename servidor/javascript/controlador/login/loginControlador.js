    ...
    ValidarCredenciales(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const SUC_DEP = yield database_1.default.query('SELECT c.id_departamento, c.id_sucursal, s.id_empresa, c.id AS id_cargo, cg_e.acciones_timbres, cg_e.public_key ' +
                ...
                if (SUC_DEP.rowCount > 0) {
                    ...
                    if (!public_key)
                        return res.status(404).jsonp({ message: 'No tiene asignada una licencia de uso de la aplicacion.' });
                    try {
                        ...
                        if (ok_licencias.length === 0)
                            return res.status(404).jsonp({ message: 'La licencia no existe, consulte a soporte tecnico' });
                        const hoy = new Date();
                ...
                let ACTIVO = yield database_1.default.query('SELECT e.estado AS empleado, u.estado AS usuario, u.app_habilita, e.codigo, e.web_access FROM empleados AS e, usuarios AS u WHERE e.id = u.id_empleado AND u.id = $1', [USUARIO.rows[0].id])
                    .then(result => {
                    return result.rows;
                });
                ...
                const [modulos] = yield database_1.default.query('SELECT * FROM funciones LIMIT 1').then(result => { return result.rows; });
                if (SUC_DEP.rowCount > 0) {
                    const { id_cargo, id_departamento, acciones_timbres, id_sucursal, id_empresa, public_key: licencia } = SUC_DEP.rows[0];
                    const AUTORIZA = yield database_1.default.query('SELECT estado FROM depa_autorizaciones WHERE id_empl_cargo = $1 AND id_departamento = $2', [id_cargo, id_departamento]);
                    if (AUTORIZA.rowCount > 0) {
                        const { estado: autoriza_est } = AUTORIZA.rows[0];
                        const token = jsonwebtoken_1.default.sign({
                            _licencia: licencia, codigo: codigo, _id: id, _id_empleado: id_empleado, rol: id_rol, _dep: id_departamento, _web_access: web_access,
                        ...
                        return res.status(200).jsonp({
                            caducidad_licencia, token, usuario: user, rol: id_rol, empleado: id_empleado, departamento: id_departamento, acciones_timbres: acciones_timbres,
